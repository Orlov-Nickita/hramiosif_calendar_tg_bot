...
from excel_utils.open_check_funcs import (
    check_new_file,
    open_to_dict,
    data_to_json,
    to_lists_of_column,
...
async def load_photo_or_doc_from_bot(
    ...
    """
    logger.info("Запущена функция load_photo_or_doc_from_bot", username=msg.from_user.username, user_id=msg.chat.id)
    ...
    try:
        ...
        msg = await bot.send_message(
            chat_id=msg.chat.id, text="Что-то случилось и я потерял файл для перезаписи. Начните, пожалуйста, сначала"
        )

        logger.info('Бот отправил сообщение\n"{}"'.format(msg.text), user_id=msg.chat.id)
        ...
        if photo:
            ...
            msg_func = await bot.send_message(chat_id=msg.chat.id, text=bot_text, reply_markup=keyboard)
            logger.info('Бот отправил сообщение "{}"'.format(msg_func.text), user_id=msg.chat.id)
            ...
            if not await check_new_file(
                ...
                data_to_json(
                    json_file=schedules_excel_dir + json_days_with_lines,
                    data_dict_or_list=all_days_in_schedule_file(jsonfile=schedules_excel_dir + json_days_list),
                )
                data_to_json(
                    json_file=schedules_excel_dir + json_days_with_index,
                    data_dict_or_list=reversed_dict_days_and_row_index(
                        data_from_json(json_file=schedules_excel_dir + json_excel_file)
                    ),
                )
                ...
                if await check_new_file(
                    ...
                    msg_func = await bot.send_message(chat_id=msg.chat.id, text="Произошла ошибка")
                    await state.finish()
            ...
            msg_func = await bot.edit_message_text(
                chat_id=msg.chat.id, message_id=msg.message_id, text=bot_text, reply_markup=keyboard
            )
            logger.info('Бот отправил сообщение "{}"'.format(msg_func.text), user_id=msg.chat.id)
