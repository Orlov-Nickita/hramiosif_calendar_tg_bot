...
async def upload_excel_file_func(message: Message, state: FSMContext) -> None:
    ...
    if message.document is None:
        ...
        msg = await bot.send_message(chat_id=message.chat.id,
                                     text='Ошибка. Нужен файл. Повторите отправку или отправьте команду "Отмена"')
        ...
        logger.error('Пользователь отправил файл с неправильным форматом {}'.
                     format(os.path.splitext(t.file_path)[1]),
                     username=message.chat.username,
                     user_id=message.chat.id)
        
        msg = await bot.send_message(chat_id=message.chat.id,
                                     text='Ошибка. Нужен файл в формате ".xlsx". Повторите отправку или отправьте '
                                          'команду "Отмена"')
...
@dp.callback_query_handler(
    lambda call: (call.data == 'yes_overwrite_excel'
...
async def overwrite_excel_func(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if call.data == 'yes_overwrite_excel':
        temp_mg = await bot.edit_message_text(chat_id=call.message.chat.id,
