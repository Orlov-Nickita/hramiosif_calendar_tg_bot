...
async def upload_sql(message: Message) -> None:
    ...
    :return: Файл с БД.
    """
    logger.info("Запущена функция admin_sql.upload_sql", username=message.chat.username, user_id=message.chat.id)

    await bot.send_chat_action(chat_id=message.chat.id, action=ChatActions.UPLOAD_DOCUMENT)

    await bot.send_document(chat_id=message.chat.id, document=open(users_sql_dir + users_sql_file_name, "rb"))

    logger.info("Бот отправил файл БД", user_id=message.chat.id)
...
async def followers_func(message: Message) -> None:
    ...
    logger.info('Бот отправил сообщение\n"{}"'.format(message.text), user_id=message.chat.id)
