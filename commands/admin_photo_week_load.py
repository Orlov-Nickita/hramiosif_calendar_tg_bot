...
async def start_week(message: Message) -> None:
    ...
    """
    logger.info(
        "Запущена функция admin_photo_week_load.start_week", username=message.chat.username, user_id=message.chat.id
    )

    await WeekPhotoDownload.get_file.set()

    msg = await bot.send_message(chat_id=message.chat.id, text="Пришли расписание на неделю и я сохраню")

    logger.info('Бот отправил сообщение "{}"'.format(msg.text), user_id=message.chat.id)


@dp.message_handler(lambda message: message.text == "Отмена" or message.text == "отмена", state=WeekPhotoDownload)
...
async def photo_week(message: Message, state: FSMContext) -> None:
    ...
    """
    logger.info(
        "Запущена функция photo_week пользователь отправил в бот файл",
    ...
    try:
        ...
        if message.photo:
            logger.info("Пользователь прислал фото", username=message.chat.username, user_id=message.chat.id)
        ...
        await bot.send_message(
            chat_id=message.chat.id, text='Ошибка. Нужно фото. Повторите отправку или отправьте команду "Отмена"'
        )

    else:
        msg = await bot.send_message(
            text="На какую неделю это расписание?", chat_id=message.chat.id, reply_markup=IKM_admin_week_save_photo()
        )
    ...
    if call.data == "this_week" or call.data == "next_week":
        ...
        try:
            ...
            await bot.send_message(
                ...
                text="Неверный формат файла, нужно фото. Повторите отправку или отправьте " 'команду "Отмена"',
            )
            ...
            if os.path.exists(src):
                ...
                await bot.send_message(
                    ...
                    text="Пользователь {id} добавил расписание на текущую неделю {week_digit}".format(
                        id=call.message.chat.id, week_digit=week_digit
                    ),
                )
                await state.finish()
...
async def download_photo_week(message: Message, state: FSMContext) -> None:
    ...
    """
    logger.info("Запущена функция download_photo_week", username=message.from_user.username, user_id=message.chat.id)
            ...
            if os.path.exists(src):
                ...
                await bot.send_message(
                    ...
                    text="Пользователь {id} добавил расписание на неделю {week_digit}".format(
                        id=message.chat.id, week_digit=message.text
                    ),
                )
                await state.finish()
...
async def overwrite_week(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if call.data == "yes_overwrite":
        ...
        await bot.send_message(
            chat_id=administrators["Никита"],
            text="Пользователь {id} перезаписал расписание на неделю".format(id=call.message.chat.id),
        )
        ...
        msg = await bot.send_photo(
            chat_id=call.message.chat.id, photo=open(src, "rb"), reply_markup=IKM_admin_overwrite_file_second_choice()
        )
...
@dp.callback_query_handler(
    lambda call: call.message.content_type != "photo" and call.data == "load_again" or call.data == "close"
)
async def repeat_save_photo(call: CallbackQuery) -> None:
    ...
    """
    if (
        "Расписание на неделю загружено" in call.message.text
        or "Файл перезаписан, расписание на неделю загружено" in call.message.text
    ):
        ...
        if call.data == "load_again":
            ...
            msg = await bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id)

            logger.info('Бот отредактировал сообщение и написал "{}"'.format(msg.text), user_id=call.message.chat.id)

    if (
        "Расписание на месяц загружено" in call.message.text
        or "Файл перезаписан, расписание на месяц загружено" in call.message.text
    ):
