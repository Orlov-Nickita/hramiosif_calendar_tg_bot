...
from keyboards_for_bot.admin_keyboards import (
    IKM_admin_check_hdd,
    IKM_open_or_not_files_in_dir_hdd,
    IKM_admin_in_dir_hdd_remove_conf,
    IKM_open_or_not_for_photo_in_dir_hdd,
)
from loader import bot, hdd_dir, administrators, users_sql_dir, users_sql_file_name, schedules_photos_dir, dp
...
async def _get_root_hdd(msg: Message, koren: bool = False, path: bool = False) -> str:
    ...
    """
    return await sql_hdd_root_dir_get(message=msg, koren=koren, path=path)
...
async def _update_root_hdd(msg: Message, direc: str, koren: bool = False, path: bool = False) -> None:
    ...
    """
    await sql_hdd_root_dir_update(root_dir=direc, koren=koren, path=path, message=msg)
...
async def _up_dir(msg: Message) -> None:
    ...
    up_dir.pop()
    up_dir = "/".join(up_dir) + "/"
    await _update_root_hdd(msg=msg, direc=up_dir, path=True)
...
async def start(message: Message) -> None:
    ...
    """
    logger.info("Запущена функция admin_hdd_check.start", username=message.chat.username, user_id=message.chat.id)
    ...
    await bot.send_message(
        ...
        text="Список файлов в папке {}".format("".join(await _get_root_hdd(msg=message, path=True))),
        reply_markup=IKM_admin_check_hdd(await what_files_in_dir("".join(await _get_root_hdd(msg=message, path=True)))),
    )


@dp.callback_query_handler(
    lambda call: ("Папка" in button_text(call) or call.data == "return_dir_back")
...
async def what_files_in_other_dir(call: CallbackQuery) -> None:
    ...
    if call.data != "return_dir_back":
        try:
            ...
            await _update_root_hdd(
                msg=call.message, direc=f"{await _get_root_hdd(msg=call.message, path=True)}{call.data}/", path=True
            )
            if not os.path.exists(await _get_root_hdd(msg=call.message, path=True)):
                raise FileNotFoundError
            ...
            await bot.edit_message_text(
                chat_id=call.message.chat.id,
                text="Список файлов в папке {}".format("".join(await _get_root_hdd(msg=call.message, path=True))),
                message_id=call.message.message_id,
                reply_markup=IKM_admin_check_hdd(
                    await what_files_in_dir("".join(await _get_root_hdd(msg=call.message, path=True)))
                ),
            )
        ...
        if await _get_root_hdd(msg=call.message, path=True) != await _get_root_hdd(msg=call.message, koren=True):
            ...
            msg = await bot.send_message(
                chat_id=call.message.chat.id, text="Это корень диска, дальше переместиться нельзя"
            )
...
@dp.callback_query_handler(
    lambda call: ("Файл" in button_text(call) or call.data == "return_dir_back")
    and "Список файлов" in call.message.text
)
        ...
        try:
            ...
            if os.path.splitext(call.data)[1] == ".jpg" or os.path.splitext(call.data)[1] == ".png":
                ...
                await bot.send_photo(
                    ...
                    photo=open("".join(await _get_root_hdd(msg=call.message, path=True)), "rb"),
...
@dp.callback_query_handler(
    lambda call: (call.data == "yes_open_hdd_file" or call.data == "no_open_hdd_file" or call.data == "remove_hdd_file")
)
async def open_or_not_file_on_hdd(call: CallbackQuery) -> None:
    ...
    if call.data == "yes_open_hdd_file":
        ...
        await bot.send_document(
            chat_id=call.message.chat.id, document=open("".join(await _get_root_hdd(msg=call.message, path=True)), "rb")
        )
...
@dp.callback_query_handler(lambda call: call.data == "yes_remove_hdd_file" or call.data == "no_remove_hdd_file")
async def remove_hdd_file(call: CallbackQuery) -> None:
    ...
    if call.data == "no_remove_hdd_file":
        ...
        try:
            deleted = "".join(await _get_root_hdd(msg=call.message, path=True))
            os.remove(deleted)

        except PermissionError as PerErr:
            await bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)

            await bot.send_message(chat_id=call.message.chat.id, text="Удалить файл не получилось")

            logger.error(f"Ошибка {PerErr}", user_id=call.message.chat.id)

        else:
            await bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
            await bot.send_message(chat_id=call.message.chat.id, text="Файл удален")

            logger.warning("Пользователь удалил файл {}".format(deleted), user_id=call.message.chat.id)


@dp.callback_query_handler(
    lambda call: (call.data == "return_for_photo_in_dir_hdd" or call.data == "remove_for_photo_in_dir_hdd")
...
async def open_or_remove_for_photo_in_dir_hdd(call: CallbackQuery) -> None:
    ...
    if call.data == "remove_for_photo_in_dir_hdd":
        ...
        await bot.send_message(
            chat_id=call.message.chat.id, text="Подтвердите удаление", reply_markup=IKM_admin_in_dir_hdd_remove_conf()
        )


@dp.callback_query_handler(lambda call: call.data == "close_hdd" and "Список файлов" in call.message.text)
async def open_or_remove_for_photo_in_dir_hdd(call: CallbackQuery) -> None:
    """
