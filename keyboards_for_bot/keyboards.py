...
def RKM_for_the_menu() -> ReplyKeyboardMarkup:
    ...
    button1 = KeyboardButton(
        "Уточнить расписание богослужений {emoji}".format(emoji=emoji.emojize(":pray:", language="alias"))
    )
...
def IKM_date_schedule_choice(days_list: list) -> InlineKeyboardMarkup:
    ...
    for every_day in range(0, len(days_list), 2):
        if every_day != days_list.index(days_list[-1]):
            ikm_date_schedule_choice.add(
                InlineKeyboardButton(text=days_list[every_day], callback_data=days_list[every_day]),
                InlineKeyboardButton(text=days_list[every_day + 1], callback_data=days_list[every_day + 1]),
            )
        else:
            ikm_date_schedule_choice.add(
                InlineKeyboardButton(text=days_list[every_day], callback_data=days_list[every_day])
    ...
    ikm_date_schedule_choice.add(InlineKeyboardButton(text="Вернуться назад", callback_data="return"))
...
def IKM_open_schedule() -> InlineKeyboardMarkup:
    ...
    :return:
    """
    ikm_open_schedule = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text="Открыть меню", callback_data="open_again")
    )
...
def IKM_photos_sliding() -> InlineKeyboardMarkup:
    ...
    item1 = InlineKeyboardButton(text="< Листать фото", callback_data="previous")

    item2 = InlineKeyboardButton(text="Листать фото >", callback_data="next")
...
def IKM_question_send() -> InlineKeyboardMarkup:
    ...
    item1 = InlineKeyboardButton(text="Отправить", callback_data="yes_send_question")

    item2 = InlineKeyboardButton(text="Отмена", callback_data="no_send_question")
